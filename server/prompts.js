...
- 雷达图必须严格按以下规范绘制，不得自行发挥：
  ...
  * 网格 4 圈（25%/50%/75%/100%）。
  * 布局防重叠（关键）：
    - 维度名称标签放在各轴顶点外侧，偏移量 = 半径 × 1.28，字体 13px，颜色 #94a3b8。
    - 分数数值放在维度名称标签的更外侧（半径 × 1.13 处），或放在数据多边形顶点内侧（半径 × 0.82 处），字体 12px 加粗，颜色 #e6e9f0。数值与维度名必须分层、不得重叠。
    - 顶部（12 点方向）和底部（6 点方向）的标签分别用 text-anchor:middle，左右两侧按象限用 start/end，避免文字超出 SVG 画布被裁剪。
    - SVG viewBox 预留足够边距（上下左右各留 ≥ 半径 × 0.45 的空白），确保最长的维度名 + 分数完整可见、不重叠、不被裁切。
...
6. 岗位匹配度：match_report → 大号匹配度徽章（match_score 分）+ 第二个 SVG 雷达图（各维度得分，轴数 = match_report.radar 实际长度，同样遵守上面的防重叠布局规范）+ 维度明细表（每行：维度名、权重、状态徽章、分数条、JD 要求 vs 简历证据对照）。
